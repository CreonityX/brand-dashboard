                    ...
                    <div key={col.id} className="flex flex-col h-full min-w-[140px]">
                        ...
                        <div className="flex-1 bg-white/[0.02] rounded-sm p-2 space-y-2 border border-white/5">
                            {tasks.filter(t => t.status === col.id).map((task) => (
                                <div key={task.id} className="group bg-zinc-900/80 border border-zinc-800 p-3 rounded-sm hover:border-zinc-600 cursor-grab active:cursor-grabbing transition-all hover:translate-y-[-2px]">
