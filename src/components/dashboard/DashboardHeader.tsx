...
import { MOCK_BRAND_CONVERSATIONS, SETTINGS_TABS, SUPPORT_TABS, CALENDAR_VIEWS, PROFILE_TABS, ANALYTICS_TABS, FINANCE_TABS, CONTENT_REVIEW_TABS, APPLICATIONS_TABS, CREATORS_TABS, CAMPAIGNS_TABS, CAMPAIGNS_LIST, INSIGHTS_TABS } from "@/lib/mock-data";
...
export function DashboardHeader() {
    ...
    const breadcrumbs = segments.map((segment, index) => {
        ...
        let label = segment === 'messages' ? 'INBOX' : segment === 'content-review' ? 'CONTENT REVIEW' : segment === 'applications' ? 'APPLICATIONS' : segment === 'creators' ? 'CREATORS' : segment === 'campaigns' ? 'CAMPAIGNS' : segment === 'insights' ? 'INSIGHTS' : segment.replace(/-/g, ' ');
        if (index > 0 && segments[index - 1] === 'campaigns' && segment !== 'campaigns') {
            const camp = CAMPAIGNS_LIST.find(c => c.id === segment);
            if (camp) label = camp.name;
        }
    ...
    if (pathname.includes('/settings')) {
    ...
    else if (pathname.includes('/campaigns') && !pathname.match(/\/campaigns\/[^/]+/)) {
        const sub = getSubLevel('tab', CAMPAIGNS_TABS, 'list');
        if (sub) breadcrumbs.push(sub);
    }
        ...
        const sub = getSubLevel('tab', APPLICATIONS_TABS, 'pending');
        if (sub) breadcrumbs.push(sub);
    }
    else if (pathname.includes('/creators')) {
        const sub = getSubLevel('tab', CREATORS_TABS, 'discover');
        if (sub) breadcrumbs.push(sub);
    }
    else if (pathname.includes('/content-review')) {
        const sub = getSubLevel('tab', CONTENT_REVIEW_TABS, 'queue');
        if (sub) breadcrumbs.push(sub);
    }
    else if (pathname.includes('/insights')) {
        const sub = getSubLevel('tab', INSIGHTS_TABS, 'industry');
        if (sub) breadcrumbs.push(sub);
    }
    else if (pathname.includes('/messages')) {
        const convId = searchParams.get('conversation');
        if (convId) {
            const creator = MOCK_BRAND_CONVERSATIONS.find(c => c.id === convId)?.creator;
            if (creator) {
                breadcrumbs.push({ label: creator.toUpperCase(), href: '', isLink: false });
            }
        }
    }
    ...
    return (
        <header className="h-16 z-50 bg-zinc-950/80 backdrop-blur-md border-b border-zinc-800 flex items-center justify-between px-6 sticky top-0">
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 text-xs font-mono overflow-hidden whitespace-nowrap mask-linear-fade">
            ...
            <div className="flex items-center gap-2 sm:gap-4 relative shrink-0">
                ...
                <div className="relative" ref={notificationRef}>
                    ...
                    {/* Popover Banner */}
                    {showNotifications && (
                        <div className="absolute right-0 top-full mt-2 w-[400px] max-h-[600px] overflow-hidden rounded-sm border border-zinc-800 bg-zinc-950/95 backdrop-blur-xl z-50 animate-in fade-in zoom-in-95 duration-200">
